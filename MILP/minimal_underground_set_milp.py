# Script to predict minimal underground reaction sets for value-added compound production
...
model = cobra.io.read_sbml_model("../underground.xml")
...
def setup_milp(model, target, remove_blocked=False, exclude_reaction_ids=set()):
    ...
    exclude_reaction_ids takes a list of reaction ids that shouldn't be considered for underground addition
    ...
    underground_y_vars = {} # 1 for 'knockin', 0 for inactive
    ...
    for reaction in [r for r in model.reactions if r.type == "underground"]:
        ...
        underground_y_vars[y_var] = reaction

    logger.debug("Control variables created")

    # Add number of underground switch contraint constraint
    underground_turn_on = model.solver.interface.Constraint(
                            optlang.symbolics.Add(*underground_y_vars), lb=0, ub=0, name="underground_reaction_constraint"
                        )
    model.solver.add(underground_turn_on)
    ...
    switch_objective=interface.Objective(underground_turn_on.expression, direction='min')
...
def run(target_metabol):
    ...
    milp.solver.constraints.underground_reaction_constraint.ub=347
...
targets=[]
with open("../yield_calculation/native_target_compounds.txt") as target_compounds:
...
for target in targets:
    ...
    try:
        r=run(target)
        results[target]={r.solver.objective.value:[ i.id for i in r.reactions if i.id.find('u0')>-1 and abs(i.flux)>1e-4]}
    except:
        results[target]="NA"
print(results)
binary_saver(results,'results/MilpResultUnderground')
